...
export default function NewsSection({ symbol }: NewsSectionProps) {
  ...
  return (
    <div className="bg-zinc-900/30 border border-zinc-800 rounded-2xl p-6 flex flex-col h-full">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          <Newspaper className="w-5 h-5 text-blue-500" />
          <h3 className="font-mono text-sm uppercase tracking-wider font-bold">
            MARKET HEADLINES <span className="text-zinc-600 font-normal">/ {symbol}</span>
          </h3>
        </div>
          ...
          {loading ? (
            ...
            news.map((item) => (
              <motion.a
                ...
                className="block p-3 rounded-xl hover:bg-zinc-800/50 transition-all border border-transparent hover:border-zinc-700/50 group"
              >
                <div className="flex justify-between items-start gap-3 mb-2">
                  <h4 className="text-xs font-medium leading-relaxed group-hover:text-blue-400 transition-colors">
                    {item.title}
                  </h4>
            ...
            <div className="flex items-center justify-center h-full text-zinc-600 text-[10px] font-mono uppercase tracking-widest">
